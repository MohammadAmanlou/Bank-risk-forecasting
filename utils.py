...
def print_eval_metrics(eval, output , dist = 1, isClassification = True):
    print (f"train RMSE score of {output['name']}: {eval.get_train_rmse_score()}")
    print (f"test RMSE score of {output['name']}: {eval.get_test_rmse_score()}")
    print (f"test R2 score of {output['name']}: {eval.get_test_r2_score()}")
    print (f"train R2 score of {output['name']}: {eval.get_train_r2_score()}")
    if(isClassification):
        print (f"percentage of predicts in train data in distance {dist} is {eval.get_data_percentage_in_dist_train(dist)}")
        print (f"percentage of predicts in test data in distance {dist} is {eval.get_data_percentage_in_dist_test(dist)}")
        print (f"percentage of predicts in train data in distance {dist-1} is {eval.get_data_percentage_in_dist_train(dist-1)}")
        print (f"percentage of predicts in test data in distance {dist-1} is {eval.get_data_percentage_in_dist_test(dist-1)}")
